...
3) It is important to know when to stop trying new blocks on the grid. As all blocks are built out of 5 grid-cells,
you can stop placing new block if the number of consecutive free cells is not divisable by 5. This was a pointer I 
...
//	private long startTimeInMs, endTimeInMs;
...
//	boolean sol = false;
	...
	public void run() {
		...
		ArrayList <PuzzleWorker> pwal = new ArrayList<PuzzleWorker>();
...
//		int quadrant = 1;
		...
		for (int i = 0; i < pGrid.getYPos();i++ ) {
			for (int j = 0; j < pGrid.getXPos(); j++ ) {
				PuzzleWorker pw = new PuzzleWorker(j, i, pcb);
				pwal.add(pw);
				//pool.execute(pw);
				futures.add(pool.submit(pw));
...
//				
		...
		for(Future fut:futures){
			System.out.println("Waiting for finishing future " + fut.toString());
			try {
				fut.get();
		...
		for(PuzzleWorker pw: pwal){
			ArrayList <char[][]> sols = pw.getSolutions();
			solutions.addAll(sols);
			
//			for(char[][] chars:sols){
//				solutions.add(chars);
//			}
			BlockContainer subBlock = pw.getBloksFromWorker();
			for (int i= 0; i < pBox.getNrOfBlocks() ; i++ ) {
				...
				for (int j = 0; j < b.getNumberOfShapes() ; j++ ) {
					...
					//b.getShape(j).getShapePlaced()
					b.getShape(j).addShapePlaces(subBlock.getBlock(i).getShape(j).getShapePlaced());
				}
			}
		}
		...
		for (int i= 0; i < pBox.getNrOfBlocks() ; i++ ) {
			...
			for (int j = 0; j < b.getNumberOfShapes() ; j++ ) {
				System.out.println("Block " + i + "  number of times shape " + j + " placed = " + b.getShape(j).getShapePlaced());
				totalOps += b.getShape(j).getShapePlaced();
			}
		}
		System.out.println("Total Number of Block Movements: " + totalOps);
		System.out.println("Total Processing time in Seconds: " + (endTimeInMs - startTimeInMs) / 1000);
